...
public class DataVisualization extends HttpServlet {
    ...
    protected void displayPage(HttpServletRequest request, HttpServletResponse response, PrintWriter pw)
        ...
        pw.print("<h2 class='title meta'><a style='font-size: 24px;'>Data Visualization</a></h2>"
                + "<div class='entry'>");
        utility.printHtml("Barchart.html");   
...
/*        try {
        ...
        try {
                    ArrayList<Review> arr1 = MySqlDataStoreUtilities.selectProductQuantityChart();
                    String reviewJson1= new Gson().toJson(arr1);
                    response.setContentType("application/json");
                    response.setCharacterEncoding("UTF-8");
                    response.getWriter().write(reviewJson1);
            ...
            try {
                    ArrayList<Review> arr2 = MySqlDataStoreUtilities.selectProductSold();
                    String reviewJson2= new Gson().toJson(arr2);
                    response.setContentType("application/json");
                    response.setCharacterEncoding("UTF-8");
